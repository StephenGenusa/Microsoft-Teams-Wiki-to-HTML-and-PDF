...
def strip_pointless_mht_msincomps(mht_file_name):
    ...
    html_file_name = os.path.join(file_path, re.sub(r'(.*)? - (\d{1,4})', r"\2 - \1", file_name))
    ...
    with open(html_file_name, "w") as output_file:
        ...
        output_file.write('<html><body style="font-size: 23px; font-family: Arial, Helvetica, sans-serif;">')
        ...
        html_text = re.sub(r'<img.*/(img-\d{1,4}-.*?\.\w{3,4}).*?>', r'<img src="\1" />', html_text)
        ...
        for loop_count in range(1, 5):
            ...
            html_text = re.sub(r'<span style="font-siz.*?">(.*)?</span>', r'\1', html_text)
...
def run_wkhtmltopdf(html_file_name):
    ...
    command = r'"C:\Program Files\wkhtmltopdf\bin\wkhtmltopdf.exe" --page-size Letter "%s" "%s" >> wkhtmlpdf.log' % (html_file_name, pdf_file_name)
    try:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(prog="Microsoft Teams Wiki to HTML and PDF by Stephen Genusa")
    parser.add_argument('--wiki-input-path', required=True, help='path to wiki mht files')
